...
def plot_model(config_inv, sid, plot_init,file_model_output,file_model_data,ax,file_model_fund = [],plot_fund = 0):
    ...
    zp = z_plot[:]
    if not plot_init:
        p2, = ax.step(mp, zp, '-', c='r', alpha=0.8, linewidth=2)
        handles.append(p2)
    ...
    #np.savetxt(dir_this+file_model_output, model, fmt=fmt)
    #print(("{:>7s}" + "{:>12s}" * 5).format('No.', 'z', 'rho', 'vs', 'vp','std'))
    ...
    if plot_fund:
        ...
        p3, = ax.step(vs,
                       ...
                       linewidth=2)
        handles.append(p3)
        labels.append('Fundamental model')

    ax.legend(handles, labels, loc='lower left')

    if plot_init:
        ax.set_title('initial model distribution')
