...
class App(object):
	...
	def __init__(self, r_flag  = 0,oldfile='None',oldkeys=[],fundfile='None',overfile = 'None',fundkeys=[],filePath='./', curveFilePath = '', freqSeries=[], cmap='viridis', vmin=None, vmax=None, url='http://10.20.11.42:8514', maxMode=0, trigerMode=False , searchStep=2, autoT=True, periodCutRate=0.125, semiAutoRange=0.1):
		...
		self.axUpload = plt.axes([0.81, 0.05, 0.1, 0.075])
		...
		self.axDelCurveInMode = plt.axes([0.7, 0.05, 0.1, 0.075])
	...
	def SaveNear1(self,event):
		curveFileName = self.fileName[:-3] + 'curve.txt'
		curveFileExist = self.get_file_list(self.curveFilePath, begin=curveFileName)

		key_this = self.fileName[-8:-3]
		key_all = []
		for file in os.listdir(self.curveFilePath):
			key_all.append(file[3:8])
	...
	def SaveNear(self,event):
		...
		key_this = self.fileName[-8:-3]
		#print(key_this)
		x = int(key_this.split('-')[0])
		y = int(key_this.split('-')[1])
		#保留两位数字

		key_find = str(x-1).zfill(2) + '-'+str(y).zfill(2)
		curve_old_File = self.curveFilePath + 'ds_'+key_find + 'curve.txt'
